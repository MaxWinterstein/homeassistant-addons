...
# Extract only the major version number to handle pre-release strings like "17.2.rc2"
# which bc cannot parse, causing a silent fallback to the wrong branch.
OS_MAJOR=$(echo "$OS_VERSION" | grep -oE '^[0-9]+')
